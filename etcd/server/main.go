...
func GetGoodsList(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Printf("trace_id: %d, get goods endpoint result: client=%s, endpoint=%s, error=%v\n", traceID, client, endpoint, err)
	if err != nil {
		res["code"] = -1
...
func ServiceDiscovery() {
	...
	for k, _ := range serviceEndpoints {
		go func(svc string) {
			...
			// 获取当前所有服务入口
			getRes, _ := cli.Get(ctx, serviceKey, etcd.WithPrefix())
			serviceLocker.Lock()
			...
			ch := cli.Watch(ctx, serviceKey, etcd.WithPrefix(), etcd.WithPrevKV())
			for v := range ch {
